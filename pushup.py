...
class PushUp:
    def __init__(self, MPIINet, scalingFactor, camera, threshold, displaySkeleton):
        ...
        self.humanPoints = [0, 1, 5, 6, 7, 13]
        self.pairs = [[2,3],[3,4],[2,5]]
    ...
    def start(self):
        while True:
            ...
            self.countPushUp(xyPoints)
            frame = cv2.putText(frame, 'Total Pushup: {}'.format(self.activityCount), (self.scalingFactor*int(3.5*self.inWidth)//8,self.scalingFactor*(7*self.inHeight)//8), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,0), 3, cv2.LINE_AA)
    ...
    def countPushUp(self, xyPoints):

        '''
        pushUpState=1 -> The person is in upright position, elbow angle>=160
        pushUpState=0 -> The person is in down position, eblow angle in b/w (160,50)
        pushUpState goes from 1->0->1 is a pushup

        '''
        if xyPoints[2] == None or xyPoints[3] == None or xyPoints[4] == None:
            handAngle = 150
        else:
            handAngle = self.getAngleBetweenPoints(xyPoints[2], xyPoints[3], xyPoints[4])
        
        if handAngle is None:
            return
        
        if handAngle >= 160 and self.activityState == 0:
            self.activityState = 1
            self.activityCount += 1
            
        elif handAngle < 97 and handAngle >= 93 and self.activityState == 1:
